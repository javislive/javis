...
export default class FlatSwiper extends PureComponent<Props> {
  ...
  _play() {
    this._timeout = setTimeout(() => {
      ...
      const page = this._nextIndex(this.state.currentPage);
      if (!this.touched) {
        this.setPage(page);
      }
      this._play();
  ...
  _nextIndex(position: number): number {
    const {
      dataSource: { length }
    } = this.state;
